...
Это шаг "Комбинация методов" (2) из описания: чистый AST даёt только
...
def resolve_call(idx: ProjectIndex, module_obj, fn, call) -> dict:
    ...
    """
    imports = idx.imports_for(module_obj)

    # 1) self.method(...) -> метод текущего класса
    if call["kind"] == "self_attr" and fn.class_name:
        key = (fn.class_name, call["attr"])
        if key in idx.methods_by_class:
            return {"target": idx.methods_by_class[key], "resolved": True, "reason": "self"}
    ...
    if call["kind"] == "attr" and call["base"] in imports:
        ...
        if key in idx.methods_by_class:
            return {"target": idx.methods_by_class[key], "resolved": True, "reason": "import+class"}
    ...
    if call["kind"] == "attr" and call["base"] in fn.local_types:
        ...
        key = (cls_name, call["attr"])
        if key in idx.methods_by_class:
            return {"target": idx.methods_by_class[key], "resolved": True, "reason": "local_type"}
        ...
        if cls_name in imports:
            ...
            if key2 in idx.methods_by_class:
                return {"target": idx.methods_by_class[key2], "resolved": True, "reason": "local_type+import"}

    # 4) вызов свободной функции (не метод), импортированной напрямую
    if call["kind"] == "name" and call["attr"] in idx.func_to_module:
        target_module = idx.func_to_module[call["attr"]]
    ...
    if call["kind"] == "name" and call["attr"] in idx.class_to_module:
        target_module = idx.class_to_module[call["attr"]]
