...
def _find_function_location(idx, qualname: str):
...
def _read_source_slice(project_root: str, file: str, lineno: int, end_lineno: int, raw: bool = False) -> str:
...
def _collect_chain_qualnames(analysis: dict) -> list[str]:
...
def _confidence_tag(qualname: str, analysis: dict) -> str:
...
def build_llm_context(idx, project_root: str, analysis: dict) -> dict:
    chain = _collect_chain_qualnames(analysis)

    nodes_with_source = []
    for q in chain:
        loc = _find_function_location(idx, q)
        confidence = _confidence_tag(q, analysis)
        if loc is None:
        ...
        source = _read_source_slice(project_root, file, lineno, end_lineno, raw=False)
        # И чистый код без номеров, чтобы LLM копировала точный синтаксис
        raw_source = _read_source_slice(project_root, file, lineno, end_lineno, raw=True)
