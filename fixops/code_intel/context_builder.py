...
class ContextBuilder:
    ...
    def render_llm_prompt(ctx: dict) -> str:
        ...
        lines.append("5. ПРАВИЛА ОБРАБОТКИ ИСКЛЮЧЕНИЙ И ТЕСТИРОВАНИЯ:")
        lines.append("   - Если бизнес-логика требует выброса исключения (например, сущность не найдена в БД), НЕ заменяй его на `return None`, `pass` или silent-игнорирование. Выбрасывай явное исключение.")
        ...
        lines.append("   - Тест должен покрывать как успешный сценарий, так и сценарий с ожидаемым исключением. Использование `monkeypatch` обязательно для мокинга зависимостей.")
