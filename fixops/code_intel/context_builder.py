...
class ContextBuilder:
    ...
    def render_llm_prompt(ctx: dict) -> str:
        ...
        lines.append("   - Оценивай семантику ошибки: если отсутствие данных или сбой зависимости является штатной ситуацией (а не критическим сбоем системы), НЕ выбрасывай прерывающее исключение. Вместо этого: залоггируй предупреждение и обеспечь graceful degradation (пропуск, дефолтное значение или продолжение работы для остальных элементов).")
        lines.append("   - Если же спецификация или бизнес-контракт явно требуют прерывания операции при невалидном состоянии (например, нарушение инвариантов, критическая ошибка валидации), тогда выбрасывай явное и семантически корректное исключение (ValueError, KeyError, RuntimeError или доменное исключение).")
        lines.append("   - При выборе стратегии ориентируйся на то, как функция используется в вызывающем коде: если вызов находится в цикле или в составе batch-операции — предпочитай пропуск с логом; если это единичная критическая операция — предпочитай fail-fast.")
        lines.append("   - Если ты добавляешь или сохраняешь `raise` в коде, ты ОБЯЗАН предоставить обновлённый код теста в блоке ```test`, который использует `with pytest.raises(ТипИсключения):` для проверки этого поведения.")
        lines.append("   - Если ты заменяешь `raise` на graceful handling, тест должен проверять, что операция завершается успешно для валидных данных, а невалидные данные обрабатываются без краха программы.")
        lines.append("   - Использование `monkeypatch` обязательно для мокинга внешних зависимостей.")
